...
from mlforgex.plots import plot_regression_metrics,plot_classification_metrics,feature_importance,create_cloud,plot_data_analysis
    ...
    # os.remove(data_path)
        ...
        if regressor:
            ...
            models={
                ...
                "SVR":SVR(),
                "CatBoostRegressor":CatBoostRegressor(verbose=0,random_state=42,loss_function="RMSE",eval_metric="RMSE",train_dir=None)
            }
            ensemble_models=create_ensemble_models(models, list(models.keys()), "regression")
            ...
            for i in tqdm(range(len(list(models))),desc="Training Models",unit="model"):
                ...
                try:
                    ...
                    print(f"Model {model_name} failed during training/evaluation: {e}")
                    continue
            regressor_ensemble_models=[i for i in model_dict if i["model"] in ["StackingEnsemble","VotingEnsemble"]]
            model_dict=[i for i in model_dict if i["model"] not in ["StackingEnsemble","VotingEnsemble"]]
            ...
            "CatBoostClassifier":CatBoostClassifier(verbose=0,random_state=42,loss_function="Logloss",eval_metric="AUC",train_dir=None),
        }   
            if nlp:
                models={
                ...
                ensemble_models=create_ensemble_models(models, list(models.keys()), "classification")
                models.update(ensemble_models)
            for i in tqdm(range(len(list(models))), desc="Training Models", unit="model"):
                model = list(models.values())[i]
                model_name = list(models.keys())[i]
                try:
                    if model_name == "CatBoostClassifier":
                        model.fit(x_tr, y_tr,eval_set=(x_val, y_val),use_best_model=True,verbose=0) # Train model
                    else:
                        model.fit(x_train, y_train) # Train model
                    ...
                    if hasattr(model,"predict_proba"):
                        ...
                        model_train_rocauc_score = np.nan
                    ...
                    if hasattr(model,"predict_proba"):
                        ...
                        model_test_rocauc_score = np.nan
                    ...
                    print(f"Model {model_name} failed during training/evaluation: {e}")
                    continue
            classification_ensemble_models=[i for i in model_dict if i["model"] in ["StackingEnsemble","VotingEnsemble"]]
            model_dict=[i for i in model_dict if i["model"] not in ["StackingEnsemble","VotingEnsemble"]]
        ...
        if regressor:
            for i in range(len(list(models))):
                ...
                if list(models.keys())[i]=="CatBoostRegressor":
                    model.fit(x_tr, y_tr,eval_set=(x_val, y_val),use_best_model=True,verbose=0)
                else:
                    model.fit(x_train, y_train) # Train model
                # Make predictions
            ...
            for i in regressor_ensemble_models:
                best_models_copy = pd.concat(
                [best_models_copy, pd.DataFrame([i])],
                ignore_index=True)
        ...
        if classification or nlp:
            ...
            for i in range(len(list(models))):
                model = list(models.values())[i]
                # print("-> "+list(models.keys())[i],flush=True)
                if list(models.keys())[i]=="CatBoostClassifier":
                    model.fit(x_tr, y_tr,eval_set=(x_val, y_val),use_best_model=True,verbose=0) # Train model
                else:
                    model.fit(x_train, y_train) # Train model
        ...
        if combined_score_ranking.shape[0] == 0:
            raise ValueError("No models available after ranking and filtering. Check data and thresholds.")
        ...
        try:
            ...
            print(f"Failed to fit selected best model {best_model_name}: {e}")
            raise
        print("Saving the model , preprocessor...")
        if best_model_name in ["CatBoostRegressor","CatBoostClassifier"]:
...
def create_ensemble_models(base_models_dict, selected_names, problem_type='classification'):
    """
...
def main():
    ...
    parser.add_argument("--rmse_prob", default=0.7,type=float, help="RMSE probability threshold")
    parser.add_argument("--f1_prob",default=0.3, type=float, help="F1 score probability threshold")
